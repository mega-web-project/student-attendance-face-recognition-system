...
import face_recognition ##helps to recognize face
import cv2 ##helps to manipulate image
import numpy as np ##use for calculating facial dimenssions
import winsound
...
## Display image when match with Database
def details(request):
    try:
        last_face = LastFace.objects.last()
...
##function to delete an item or a student from the system
def delete_profile(request,id):
    profile = Profile.objects.get(id=id)
    profile.delete()
    return redirect('profiles')
##function to clearing the history
def clear_history(request):
    history = LastFace.objects.all()
    history.delete()
    return redirect('index')

##function to reset or clear for default
def reset(request):
    profiles = Profile.objects.all()
    for profile in profiles:
        if profile.present == True:
